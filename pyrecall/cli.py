...
def delete(
    snapshot_name: Annotated[
        str | None,
        typer.Argument(
            help="Name of a single snapshot to delete. Omit to use --all, "
            "--keep-last, or --older-than."
        ),
    ] = None,
    all_snapshots: Annotated[
        bool,
        typer.Option("--all", help="Delete every snapshot."),
    ] = False,
    keep_last: Annotated[
        int | None,
        typer.Option(
    ...
    """
    Delete snapshots and manage snapshot cleanup.

    Four modes (pick one):

        pyrecall delete before_v1              # delete one named snapshot
        pyrecall delete --all                  # delete every snapshot
        pyrecall delete --keep-last 3          # keep 3 most recent, delete the rest
        pyrecall delete --older-than 30        # delete snapshots older than 30 days

    Add --dry-run to preview without deleting. This cannot be undone.
    Pass --yes to skip the confirmation prompt, which is useful in
    non-interactive scripts and CI pipelines.
    """
    bulk_mode = all_snapshots or keep_last is not None or older_than is not None

    if snapshot_name is not None and bulk_mode:
        console.print(
            "[red]Error:[/red] Cannot combine a snapshot name with "
            "--all, --keep-last, or --older-than."
        )
        raise typer.Exit(1)

    if snapshot_name is None and not bulk_mode:
        console.print(
            "[red]Error:[/red] Provide a snapshot name, --all, --keep-last, or --older-than.\n"
            "Run [bold]pyrecall delete --help[/bold] for usage."
        )
        raise typer.Exit(1)
    ...
    if snapshot_name is not None:
        # Single-name mode: preserves legacy behaviour of clearing (rather than
        # blocking) the baseline when the named snapshot happens to be it.
        if not mgr.has_snapshot(snapshot_name):
            console.print(
                f"[red]Error:[/red] Snapshot '{snapshot_name}' not found.\n"
    ...
    if protect_baseline:
        ...
        if baseline_candidates and not force:
            console.print(
                ...
                "Pass [bold]--force[/bold] to include it in the deletion."
            )
            candidates = [s for s in candidates if s.name != baseline]
            if not candidates:
                console.print("[green]Nothing left to delete after baseline exclusion.[/green]")
                return
    ...
    console.print(
        f"\n[green]✓ Deleted {deleted} snapshot{'s' if deleted != 1 else ''}.[/green] "
